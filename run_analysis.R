...
## split the each measurement into a separate features, assign the features names and substruct the means and standrad deviations (do the same actions for test and train data sets)
testMeasures <- str_split_fixed(testSetData$measure, "[0-9] ", length(featuresData[,1]))
testMeasures<-data.frame(testMeasures, stringsAsFactors = FALSE)
names(testMeasures)<-featuresData[,1]
testMeasures<-testMeasures[,grepl( "MEAN|STD", toupper(names(testMeasures)))==TRUE]
names(testMeasures)<-sub("[0-9]* ", "",names(testMeasures))
testMeasures<-data.frame(sapply(testMeasures,as.numeric))


trainMeasures <-  str_split_fixed(trainSetData$measure, "[0-9] ", length(featuresData[,1]))
trainMeasures<-data.frame(trainMeasures, stringsAsFactors = FALSE)
names(trainMeasures)<-featuresData[,1]
trainMeasures<-trainMeasures[,grepl( "MEAN|STD", toupper(names(trainMeasures)))==TRUE]
...
## binding the dataframes for (activities and subjects) to the measurements dataframes
...
## merging the activities with activity label in order to get the labels and selecting only the intersting columns in a correct order
activities <-  merge(activities, activity_labels, by = "activity.id", all = TRUE) %>% select(-(activity.id)) %>% arrange(subject,activity.name)

##clean up unused variables
rm(list=c("testSetData", "trainSetData", "featuresData", "activity_labels", "testLblData"
          , "testSubjectData",  "trainLblData", "trainSubjectData"
          , "trainActivities", "testActivities", "trainMeasures", "testMeasures"))
...
averageActivities<-  aggregate(activities[,2:dim(activities)[2]-2], list(activities$activity.name, activities$subject), mean)
